...
/// Immutable; derive narrower identities with <see cref="ForWorkflow"/>, <see cref="ForNode"/> and
/// <see cref="ForChildExecution"/>.
/// </summary>
public sealed record ExecutionIdentity
{
    ...
    /// <param name="parentExecutionId">The invoking execution, for nested (invoked) workflows.</param>
    ...
    /// <summary>The invoking execution, for nested (invoked) workflows.</summary>
    public ExecutionId? ParentExecutionId { get; }

    /// <summary>Creates an identity for a new top-level execution.</summary>
    ...
    public static ExecutionIdentity CreateNew(IIdGenerator ids, CorrelationId? correlationId = null)
    {
        ArgumentNullException.ThrowIfNull(ids);
        return new(ids.NewExecutionId(), correlationId ?? ids.NewCorrelationId());
    }
    ...
    public ExecutionIdentity ForWorkflow(WorkflowId workflowId)
    {
        ...
        return new ExecutionIdentity(ExecutionId, CorrelationId, workflowId, NodeId, ParentExecutionId);
    }
    ...
    /// <summary>
    /// Returns the identity of a nested execution invoked from this one: new execution id, same correlation,
    /// this execution as parent, no node yet.
    /// </summary>
    /// <param name="childExecutionId">The nested execution's id.</param>
    /// <param name="childWorkflowId">The invoked workflow.</param>
    public ExecutionIdentity ForChildExecution(ExecutionId childExecutionId, WorkflowId childWorkflowId)
    {
        ArgumentNullException.ThrowIfNull(childExecutionId);
        ArgumentNullException.ThrowIfNull(childWorkflowId);
        return new ExecutionIdentity(childExecutionId, CorrelationId, childWorkflowId, nodeId: null, ExecutionId);
    }
    ...
    public IReadOnlyList<KeyValuePair<string, object?>> ToTags()
    {
        ...
        {
            tags.Add(new(DiagnosticNames.ParentExecutionIdKey, ParentExecutionId.ToString()));
        }
